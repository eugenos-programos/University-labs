...
void objed(vector<int> A, vector<int> B,vector<int> C) {  //вводимые данные функции - два множества,над которыми требуется произвести операцию, и третье - результат операции
    ...
    for (int i = 0; i < B.size(); i++) { // Берём элемент из В
        for (int j = 0; j < A.size(); j++) {   //Берём элемент из А
            if(A[j] != B[i] && (j != A.size() - 1)) continue;  // Если не равны элементы и элемент а
            if(A[j] == B[i]) break;
            if(A[j] != B[i] && (j == A.size() - 1))C.push_back(B[i]);
        }
    }
...
void raznost(vector<int> A, vector<int> B, vector<int> E) {
    for (int i = 0; i < A.size(); i++) {  // Берём элемент из множества А
        for (int j = 0; j < B.size(); j++) {  //Последовательно сравниваем его с элементами из B
            if(A[i] != B[j] && (j+1 != B.size()))continue;  // Если они не равны и элемент множества B не последний - переходим к след. элементу B
            if(A[i] != B[j] && (j+1 == B.size()))E.push_back(A[i]);  // Если не равны и последний - добавляем и переходим к след.элементу B
            if(A[i] == B[j])break;  // если равны переходим к след.элементу множества B
        }
    }
...
void simm_raznost(vector<int> A, vector<int> B, vector<int> F) {
    for (int i = 0; i < A.size(); i++) {  // Берём элемент из множества А
        for (int j = 0; j < B.size(); j++) {  //Последовательно сравниваем его с элементами из B
            if(A[i] != B[j] && (j+1 != B.size()))continue;  //Если не равны и не последний - берём след.элемент множества B
            if(A[i] != B[j] && (j+1 == B.size()))F.push_back(A[i]);  //Если не равны и последний элемент мн.В - добалвяем и переходим к след.элементу множества А
            if(A[i] == B[j])break;  // если равны переходим к след.элементу множетсва А
        }
    }
    for (int i = 0; i < B.size(); i++) {  // Берём элемент из множества В
        for (int j = 0; j < A.size(); j++) {  //Последовательно сравниваем его с элементами из А
            if(B[i] != A[j] && (j+1 != A.size()))continue;    //Если не равны и не последний - берём след.элемент множества А
            if(B[i] != A[j] && (j+1 == A.size()))F.push_back(B[i]);   //Если не равны и последний элемент мн.А - добалвяем и переходим к след.элементу множества В
            if(B[i] == A[j])break;  // если равны переходим к след.элементу множетсва В
        }
    }
