...
namespace oriented_graph
{
    ...
    */
    ...
    public:
        ...
        typedef std::pair<value_type, value_type> ArcType;
    ...
    public:
         ...
         */
        VertexIterator avEnd(const value_type &_vertex)
        {
            std::vector<value_type> _adjVertexList = getAdjacencyVertices(_vertex);
            if (_adjVertexList.empty())
                throw "Empty adjacency vertex list";
         ...
         * @return const iterator on the end of vertex list
         */
        ConstVertexIterator cavEnd(const value_type &_vertex)
        {
            std::vector<value_type> _adjVertexList = getAdjacencyVertices(_vertex);
            if (_adjVertexList.empty())
         ...
         */
        ArcIterator aiEnd(const value_type &_vertex)
        {
            std::vector<ArcType> _incidenceArcList = getAllIncidenceArcs(_vertex);
            if (_incidenceArcList.empty())
                throw "No incidence arcs";
         ...
         */
        ReverseArcIterator raiEnd(const value_type &_vertex)
        {
            std::vector<ArcType> _incidenceArcList = getAllIncidenceArcs(_vertex);
            if (_incidenceArcList.empty())
                throw "No incident arcs";
         ...
         */
        ConstArcIterator caiEnd(const value_type &_vertex)
        {
            std::vector<ArcType> _incidenceArcList = getAllIncidenceArcs(_vertex);
            if (_incidenceArcList.empty())
                throw "No incident arcs";
    ...
    {
        out << "Vertices:\n";
        /*std::for_each(_graph.vBegin(), _graph.vEnd(), [](T name){std::cout << name << ' ';});*/
        return out << endl;
    }
}
